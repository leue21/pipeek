(() => {
  ...
  let failed = false;
  const updateStatus = () => {
    const sampled = Number(metrics.querySelector('[data-sampled]')?.dataset.sampled);
    const stale = !sampled || Date.now() - sampled > interval * 3;
  ...
  document.body.addEventListener('htmx:afterSwap', updateStatus);
